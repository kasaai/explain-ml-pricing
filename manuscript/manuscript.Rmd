...
We motivate our discussion by considering several aspects of interpretability. As we proceed through the points below, we aim to arrive at a more scoped and relevant definition of what it means to attain interpretability in a pricing model. We first clarify a couple concepts regarding interpretable *classes* of models and and the computational transparency of ML models. We then emphasize findings on the social aspects of ML explanations most relevant to our use case. With the provided background, we discuss a potential framework for implementing ML interpretability in practice.

## Not All Linear Models are Interpretable {#linear-models}

Let us look at some examples of models that are considered by many to be interpretable. The GLM is probably the most oft-cited example of such a model (or rather, modeling framework) in the actuarial literature. Given a set of inputs, we can easily reason about what the output of the model is. As an illustrative example, we may have a claims severity model with driver age, amount of insurance (AOI), and sex as features; assuming a log link function, we have

\begin{equation}
  \log(E(\text{severity})) = \beta_0 + \beta_1 \cdot \text{age} + \beta_2 \cdot \text{AOI} + \beta_3 \cdot \text{sex}_{\text{male}}.
\end{equation}
...
With the above considerations in mind, we propose a potential framework for interpreting ML models for insurance pricing: the actuarial profession, in collaboration with regulators, define a set of questions to be answered by explanations accompanying ML models, along with acceptance criteria and examples of successful explanations. In other words, interpretability for our purposes is defined as the ability of a model's explanations to answer the posed questions.
...
Now that we have established a framework for model interpretation in the form of asking and answering relevant questions, we demonstrate examples of such exchanges via an illustrative case study. Analytically, our starting point is a fitted deep neural network model for predicting loss costs. As the modeling details are of secondary importance, they are available in the appendix (TODO: add appendix and link to it.) The questions that we ask of the model are as follows:
...
(TODO: add citation to interpretable ml book)

Before we dive into the answering questions, we provide some background on model interpretation techniques. As the literature is quite sizeable and fast moving, we do not attempt a full taxomony of techniques. The goal is to orient ourselves among broad categories of techniques, so we can map them to tasks indicated by the questions being asked. We classify model interpretation techniques across two dimensions: intrinsic vs. post-hoc and global vs. local.

### Intrinsic vs. Post-hoc

Intrinsic model interpretation draws conclusions from the structure of the fitted model and are what we typically associate with "interpretable" classes of models. This is only viable with models with simple structures, such as the sparse linear model and shallow decision tree we see in Section \ref{linear-models}, where we arrive at explanations by reading off parameter estimates or a few decision rules. For algorithms that produce models with complex structure that do not lend themselves easily to intrinsic exploration, we can appeal to post-hoc techniques. This class of techniques interrogate the model by presenting it with data for and observing the prediction behavior of the model. These techniques are concerned with only the inputs and outputs, and hence are *agnostic* of the model itself, which means they can be applied also to simple models. Since most useful ML models have a level of complexity beyond the threshold of intrinsic interpretability, we focus on model-agnostic techniques in our case study. As we will see later on, the data that we present to the models are usually some perturbed variations of test data.
...
Having aligned the questions with the categories of interpretation techniques, we now introduce a sample of appropriate techniques to answer them.
...
In Figure \@ref(fig:fi), we show a plot of variable importances. We see that FOO and BAR are the most important variables in terms of contributing accuracy to our model. Note that these measures do not provide information regarding the directional sensitivity of the predictors on the response. In our particular example, we see that the "make" variable contributes most to the accuracy of the model with "sex" contributing the least. This provides a way for the audience to quickly glance at the most relevant variables, and ask further question as necessary. For example, if a variable that can potentially be a proxy for a discriminatory variable appears as very important, more investigation would be warranted.
...
To describe PDP, we first introduce some notation. Let $x^j$ denote the input variable of interest. Then we define the partial dependence function as
...
In Figure \@ref(fig:pdp), we exhibit the PDP for the "vehicle age" variable. We see that the average predicted loss cost decreases with vehicle age until the latter is around 18. Note that the ticks on the axis tell us that the data is quite thin for vehicle age > 18, so the apparent upward trend to the right is driven by just a few data points. With this in mind, we can conclude that as cars get older, we expect the loss cost predictions generated by our model to decrease.
...
\begin{equation}\label{eq:var-contrib}
v(j, x_* ) = E_X[\widehat{f}(X) | X^1 = x_* ^1, \dots, X^j = x_* ^j] - E_X[\widehat{f}(X^1 = x_* ^1, \dots, X^{j-1} = x_*^{j - 1})].
\end{equation}
