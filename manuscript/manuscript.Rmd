...
the syllabus of basic education does not seem to include any mention of GLMs 
prior to @brown_1988 in the 1990 syllabus for basic education. From there, GLMs 
...
More recently, the MAS-I and MAS-II exams introduced in 2018 have included 
...
predictive models may be subject to intense regulatory scrutiny. In these cases,
...
the results from the methods found in the syllabus. [This can be supported by 
reference to the edits to ASOP 38 over time - originally it was designed to be 
about all models, but they revised it to be about only models that incorporate 
specialized knowledge outside of the actuary's area of expertise... do we need 
this citation, though?] This is notable because, beginning with the introduction
of the MAS-I and MAS-II examination in July of 2018, several machine learning 
...
41 for several reasons. Machine learning models can be very ad hoc compared to 
traditional statistical models. Because many machine learning models are 
deterministic, they may not admit of standard metrics for model comparison 
(e.g., it's not straightforward to calculate an AIC over a neural network). 
In addition, machine learning methods are often combined into ensembles that 
may not be easily separated and that may, as a collection, cease to resemble 
a single standard version of a model. Complicating matters still further, 
...
of the model. More information is necessary.

Of course, these comments only apply to the actuarial profession. Outside of 
...
the National Association of Insurance Commissioners found that the plurality of
...
* Determine the extent the model causes premium disruption for individual 
...
relevant definition of what it means to attain interpretability in a pricing
model. In the remainder of this section, we clarify a couple concepts regarding 
interpretable *classes* of models and and the computational transparency of ML
models, outline findings on the social aspects of ML explanations most relevant
to our use case, then discuss a potential framework for implementing ML 
...
features; assuming a log link function and letting $Y$ denotes the response, we
have
...
because the relationship between the feature and the response is simply 
...
only a handful  of features. In fact, the ease at which we can reason about a
model declines as the number of features, transformations of them, and
...
Proposed by @hiltonConversationalProcesses1990 and interpreted by
@millerExplanationArtificial2017 in the context of ML, model explanations
are conversations or social interactions. One consequence of this identification
is that explanations need to be *relevant* to the *audience*. In developing,
...
and public policy. Finally, we have the public, who has an interest (perhaps
more so than the other parties) because their lives are directly affected by
...
which may be lost on the prospective policyholder.
...
standardizing on a set of questions, we reduce the burden of both companies and
...
case of GLMs, one can observe the magnitudes of the $t$-statistics, whereas
for random forests, one can use out-of-bag errors [@breimanRandomForests2001].
...
matrix where the $j$th variable has been permutated, then we can compute the
...
further question as necessary. For example, if a variable that can potentially
be a proxy for a discriminatory variable appears as very important, more
...
trend to the right is driven by just a few data points. With this in mind,
we can conclude that as cars get older, we expect the loss cost predictions
generated by our model to decrease.
...
practices for supervised learning model development. However, for the sake of 
...
point in $\mathbb{R}^2$ through embedding layers. The model is a feedforward
