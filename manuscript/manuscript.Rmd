...
the syllabus of basic education of the Casualty Actuarial Society does not seem to include any mention of GLMs 
...
case of GLMs, one might observe the magnitudes of the $t$-statistics, step-wise
reduction in information criteria, or out-of-sample performance, whereas
...
Note that this proposed plot can be calculated over the dataset or over a
hypothetical dataset in which the value of $x_i^{j}$ is set equal to $z$ for
each record.
...
In this paper, we discussed particular questions of model validation that are
...
We contextualized these questions in terms of different frameworks for defining
...
refined. We then offered potential (families of) model-agnostic techniques for
...
nlocking the potential of machine learning within the insurance industry.
